...
class _CrewFormDialogState extends State<_CrewFormDialog> {
  ...
  // Регулярний вираз: тільки англійські літери (великі та малі)
  ...
  // Метод для отримання списку ліцензій, дозволених для обраної посади
  List<Map<String, dynamic>> get _availableLicenses {
    ...
    if (posName == 'Pilot' || posName == 'Co-Pilot') {
      return widget.licenseTypes.where((l) {
        ...
        return name.contains('Pilot License'); // PPL, CPL, ATPL
      }).toList();
    }
    ...
    // Для Flight Attendant можна або повернути пустий список, або спеціальну ліцензію
  ...
  Future<void> _save() async {
    ...
    try {
      ...
      // Якщо бекенд все ж повернув помилку, виводимо її через SnackBar 
      // (або можна розпарсити JSON і передати в FormField errorText)
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
        ...
        return CustomSelectField(
          ...
          onChanged: (val) {
            setState(() {
              ...
              // При зміні посади скидаємо ліцензію, якщо вона більше не підходить
              _licenseTypeId = null; 
  ...
  Widget _buildLicenseSelect() {
    ...
    return FormField<int>(
      key: ValueKey('pos_$_positionId'), // Перестворюємо поле при зміні посади
